...
function splitDeck() {
  ...
  // 1st bidder (player 3): spades 1-12
  // 2nd bidder (player 2): clubs 1-12
  // 3rd bidder (player 1): diamonds 1-12
...
test('cards get placed at the right player-index positions (faithful-replay anchoring)', () => {
  const r = reconstructDeck(splitDeck());
  const url = r.url;
  // Engine bid order with dealer at index 0 is 3 → 2 → 1 → 0, so the
  // 1st bidder anchors at player index 3 and the 3rd bidder at index 1.
  // Dealer (player 0) has hearts 1-12 → positions 0,4,8,...,44
  for (let i = 0; i < 48; i += 4) {
    assert.match(url[i], /[a-l]/, `pos ${i} should be hearts 1-12, got ${url[i]}`);
  }
  // 3rd bidder (player 1) has diamonds → positions 1,5,9,...,45
  ...
  // 1st bidder (player 3) has spades → positions 3,7,...,47
