...
const ENTRY = path.join(ROOT, 'src', 'simulation', 'runDominanceCheck.ts');
const OUT_DIR = path.join(ROOT, 'build-scripts');
const OUT = path.join(OUT_DIR, 'dominance-check.bundle.js');
...
async function main() {
  console.log('Bundling dominance check...');
  await build();
