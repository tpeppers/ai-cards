...
const ENTRY = path.join(ROOT, 'src', 'simulation', 'runClaudeFamBenchmark.ts');
const OUT_DIR = path.join(ROOT, 'build-scripts');
const OUT = path.join(OUT_DIR, 'claudefam-benchmark.bundle.js');
...
async function main() {
  console.log('Bundling ClaudeFam benchmark...');
  await build();
