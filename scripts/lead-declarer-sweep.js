...
const ENTRY = path.join(ROOT, 'src', 'simulation', 'runLeadDeclarerSweep.ts');
const OUT_DIR = path.join(ROOT, 'build-scripts');
const OUT = path.join(OUT_DIR, 'lead-declarer-sweep.bundle.js');
...
async function main() {
  console.log('Bundling lead-declarer sweep...');
  await build();
