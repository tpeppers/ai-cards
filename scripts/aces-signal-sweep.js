...
const ENTRY = path.join(ROOT, 'src', 'simulation', 'runAcesSignalSweep.ts');
const OUT_DIR = path.join(ROOT, 'build-scripts');
const OUT = path.join(OUT_DIR, 'aces-signal-sweep.bundle.js');
...
async function main() {
  console.log('Bundling aces-signal sweep...');
  await build();
