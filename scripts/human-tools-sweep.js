...
const ENTRY = path.join(ROOT, 'src', 'simulation', 'runHumanToolsSweep.ts');
const OUT_DIR = path.join(ROOT, 'build-scripts');
const OUT = path.join(OUT_DIR, 'human-tools-sweep.bundle.js');
...
async function main() {
  console.log('Bundling Roles benchmark...');
  await build();
