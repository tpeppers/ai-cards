...
const ENTRY = path.join(ROOT, 'src', 'simulation', 'runAcesFullSweep.ts');
const OUT_DIR = path.join(ROOT, 'build-scripts');
const OUT = path.join(OUT_DIR, 'aces-full-sweep.bundle.js');
...
async function main() {
  console.log('Bundling aces-full sweep...');
  await build();
