...
const ENTRY = path.join(ROOT, 'src', 'simulation', 'runStopperDiagnostic.ts');
const OUT_DIR = path.join(ROOT, 'build-scripts');
const OUT = path.join(OUT_DIR, 'stopper-diagnostic.bundle.js');
...
async function main() {
  console.log('Bundling stopper diagnostic...');
  await build();
