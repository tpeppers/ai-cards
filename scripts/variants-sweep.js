...
const ENTRY = path.join(ROOT, 'src', 'simulation', 'runPlayVariantsSweep.ts');
const OUT_DIR = path.join(ROOT, 'build-scripts');
const OUT = path.join(OUT_DIR, 'variants-sweep.bundle.js');
...
async function main() {
  console.log('Bundling variants sweep...');
  await build();
