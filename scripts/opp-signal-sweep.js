...
const ENTRY = path.join(ROOT, 'src', 'simulation', 'runOppSignalSweep.ts');
const OUT_DIR = path.join(ROOT, 'build-scripts');
const OUT = path.join(OUT_DIR, 'opp-signal-sweep.bundle.js');
...
async function main() {
  console.log('Bundling opp-signal sweep...');
  await build();
